...
enum result get_file_name_list(struct table **table, struct file_name_list **file_nameList)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        char test_file[255] = "music.mp3";
        memcpy(file_name.file_name, test_file, 255);
...
void *host_scaner(void *arg)
{
   ...
   /*
   printf("ARG ADDR %p ", arg);// АДРЕС  который хранит указатель
   printf("TEST ADDR %p\n", test);// АДРЕС  который хранит указатель
   printf("MUSOR\n");
   printf("N = %d: %s %p\n", test->user.i, test->user.ip, test->next);
    */
